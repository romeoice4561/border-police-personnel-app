...
/** Builds a Timeline row with the Phase 23A source/verified fields defaulted, so fixtures only specify what they test. */
function timelineRow(ov: Partial<Timeline> & { id: number; officerId: number; sequence: number; year: string; position: string }): Timeline {
  return { yearValue: null, rank: null, unit: null, source: null, verified: "ยังไม่ตรวจ", ...ov };
}
