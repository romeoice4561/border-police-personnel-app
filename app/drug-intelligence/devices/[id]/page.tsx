/**
 * Device/IMEI entity detail (Phase DI-3 — Section 15). Brand/model/IMEI1/
 * IMEI2/serial, related persons, source cases. "พบใช้งานร่วมกับ" for
 * person-device links — never "owner" (Section 17).
 */
"use client";

import { useParams } from "next/navigation";
import Link from "next/link";
import { ArrowLeft, Smartphone, Users, FileSpreadsheet, Network, History } from "lucide-react";
...
export default function DrugDeviceDetailPage() {
  ...
  const title = [data.device.brand, data.device.model].filter(Boolean).join(" ") || (data.device.imei1 ? presentIdentifierValue(data.device.imei1, canViewFull) : t("di.entity.deviceTitle"));

  return (
    <div className="space-y-5">
      <PageHeader
        title={title}
        description={t("di.entity.deviceTitle")}
        actions={
          <div className="flex flex-wrap gap-2">
            <Button asChild variant="outline" size="sm">
              <Link href={`/drug-intelligence/network?focusType=DEVICE&focusId=${encodeURIComponent(deviceId)}`}>
                <Network className="h-4 w-4" aria-hidden="true" />
                {t("di.network.openNetwork")}
              </Link>
            </Button>
            <Button asChild variant="outline" size="sm">
              <Link href={`/drug-intelligence/timeline?deviceId=${encodeURIComponent(deviceId)}`}>
      ...
      <Card>
        <CardBody className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm sm:grid-cols-4">
          <span className="text-muted">
            {t("di.entity.imei1")}: <span className="text-foreground">{data.device.imei1 ? presentIdentifierValue(data.device.imei1, canViewFull) : "—"}</span>
          </span>
          <span className="text-muted">
            {t("di.entity.imei2")}: <span className="text-foreground">{data.device.imei2 ? presentIdentifierValue(data.device.imei2, canViewFull) : "—"}</span>
          </span>
          <span className="text-muted">
            {t("di.entity.serialNumber")}: <span className="text-foreground">{data.device.serialNumber ? presentIdentifierValue(data.device.serialNumber, canViewFull) : "—"}</span>
          </span>
          <span className="text-muted">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {data.relatedPersons.map((person) => (
              <Link key={person.id} href={`/drug-intelligence/persons/${encodeURIComponent(person.id)}`} className="block rounded-xl border border-border bg-surface p-4 hover:border-accent/50">
                ...
                <p className="mt-1 text-xs text-muted">{t("di.entity.foundInUse")}</p>
              </Link>
            ))}
          </div>
        )}
