/**
 * Vehicle entity detail (Phase DI-3 — Section 16). Registration/province/
 * VIN/brand/model/color, related persons, source cases. "พบเกี่ยวข้องกับ" —
 * never "owner" (Section 17).
 */
"use client";

import { useParams } from "next/navigation";
import Link from "next/link";
import { ArrowLeft, Car, Users, FileSpreadsheet, Network, History } from "lucide-react";
...
export default function DrugVehicleDetailPage() {
  ...
  const title = data.vehicle.registrationNumber
    ? `${presentIdentifierValue(data.vehicle.registrationNumber, canViewFull)}${data.vehicle.registrationProvince ? ` ${data.vehicle.registrationProvince}` : ""}`
    : t("di.entity.vehicleTitle");
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            ...
            <Button asChild variant="outline" size="sm">
              <Link href={`/drug-intelligence/timeline?vehicleId=${encodeURIComponent(vehicleId)}`}>
      ...
      <Card>
        <CardBody className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm sm:grid-cols-4">
          <span className="text-muted">
            {t("di.entity.registrationNumber")}: <span className="text-foreground">{data.vehicle.registrationNumber ? presentIdentifierValue(data.vehicle.registrationNumber, canViewFull) : "—"}</span>
          </span>
          <span className="text-muted">
            {t("di.entity.registrationProvince")}: <span className="text-foreground">{data.vehicle.registrationProvince ?? "—"}</span>
          </span>
          <span className="text-muted">
            {t("di.entity.vin")}: <span className="text-foreground">{data.vehicle.vin ? presentIdentifierValue(data.vehicle.vin, canViewFull) : "—"}</span>
          </span>
          <span className="text-muted">
            {t("di.entity.color")}: <span className="text-foreground">{data.vehicle.color ?? "—"}</span>
          </span>
          <span className="text-muted">
      ...
      <p className="text-xs text-muted">
        <Car className="mr-1 inline h-3.5 w-3.5" aria-hidden="true" />
