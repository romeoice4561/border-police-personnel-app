/**
 * Entity picker with optional type filter (DI-5 + Phase 1B Relationship Search).
 * Reuses Global Search — never a second search engine.
 */
"use client";
...
import type { DrugGraphNodeType, DrugSearchEntityType } from "@/lib/drug_intelligence/drug_intelligence_client";
  ...
  return (
    <div className="relative">
      ...
      {query.trim() && search.data && search.data.totalCount > 0 ? (
        <div className="absolute z-10 mt-1 max-h-72 w-full overflow-y-auto rounded-lg border border-border bg-surface shadow-lg" role="listbox">
          {search.data.groups
            .filter((group) => !allowed || allowed.has(group.entityType as DrugGraphNodeType))
            .map((group) => (
              <div key={group.entityType}>
                <p className="bg-neutral-bg px-3 py-1 text-xs font-semibold uppercase tracking-wide text-muted">
                  {t(DRUG_GRAPH_NODE_TYPE_LABEL_KEY[group.entityType as DrugGraphNodeType] as TranslationKey)}
                </p>
                {group.results.map((result) => (
                  <button
                    ...
                    className="block w-full px-3 py-2.5 text-left text-sm text-foreground hover:bg-neutral-bg"
                  >
