  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-3">
        <CardTitle>{t("officer.training")}</CardTitle>
      ...
      <CardBody className="space-y-3">
        {rows.length === 0 ? (
          <p className="py-6 text-center text-sm text-muted">{t("officer.trainingEditorEmpty")}</p>
        ) : (
          rows.map((row) => (
            <div key={row.key} className="grid grid-cols-1 gap-3 rounded-xl border border-border p-4 sm:grid-cols-2">
              <div>
                <label className="mb-1.5 block text-xs font-medium text-muted">{t("officer.trainingEditorYear")}</label>
                <input
                  type="text"
                  className={inputCls}
                  ...
                  aria-label={t("officer.trainingEditorYear")}
                />
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-medium text-muted">{t("officer.trainingEditorOrganization")}</label>
                <input
                  type="text"
                  className={inputCls}
                  ...
                  aria-label={t("officer.trainingEditorOrganization")}
                />
              </div>
              <div className="sm:col-span-2">
                <label className="mb-1.5 block text-xs font-medium text-muted">{t("officer.trainingEditorCourse")}</label>
                <input
                  type="text"
                  className={inputCls}
                  ...
                  aria-label={t("officer.trainingEditorCourse")}
                />
              </div>
              <div className="sm:col-span-2">
                <label className="mb-1.5 block text-xs font-medium text-muted">{t("officer.trainingEditorNotes")}</label>
                <textarea
                  rows={2}
                  className={`${inputCls} resize-y`}
                  value={row.notes}
                  onChange={(e) => updateRow(row.key, { notes: e.target.value })}
                  aria-label={t("officer.trainingEditorNotes")}
                />
              </div>
